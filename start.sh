...
#             Dead simple: no extra venv, no MPS dtype dragons. macOS only.
#   comfyui — ComfyUI in a Python venv with SDXL/Flux .safetensors checkpoints.
#             More flexible but more moving parts (and we already paid in tears).
#
# Auto-detect: if user hasn't set IMAGE_BACKEND and ~/ComfyUI exists, stay on
# comfyui (no surprise migration of a working install). Fresh installs default
# to ollama. Override anytime by setting IMAGE_BACKEND in .env.
...
case "$IMAGE_BACKEND" in
    ollama|comfyui) ;;
    *) echo "Error: IMAGE_BACKEND must be 'ollama' or 'comfyui' (got: $IMAGE_BACKEND)"; exit 1 ;;
esac

if [ "$IMAGE_BACKEND" = "ollama" ]; then
    if [ "$OS" != "Darwin" ]; then
        echo "Error: IMAGE_BACKEND=ollama is currently macOS-only (Ollama image gen)."
        echo "       Set IMAGE_BACKEND=comfyui in .env on Linux."
        exit 1
    fi
    if [ "$OLLAMA_MODE" != "native" ]; then
        echo "Error: IMAGE_BACKEND=ollama requires OLLAMA_MODE=native."
        echo "       Docker-mode Ollama is CPU-only; image gen would be impractical."
        exit 1
    fi
fi
...
#   comfyui: preset key (sdxl|flux-schnell|flux-dev) → checkpoint + workflow
#   ollama:  Ollama model tag (e.g. x/flux2-klein:4b, x/z-image-turbo:fp8)
case "$IMAGE_BACKEND" in
    comfyui)
        IMAGE_MODEL="${IMAGE_MODEL:-sdxl}"
        case "$IMAGE_MODEL" in
            sdxl)         CKPT_NAME="${COMFYUI_DEFAULT_MODEL_NAME:-sd_xl_base_1.0.safetensors}";  WF_CFG=7.0;  WF_STEPS=20; WF_SCHEDULER=normal ;;
            flux-schnell) CKPT_NAME="${COMFYUI_DEFAULT_MODEL_NAME:-flux1-schnell-fp8.safetensors}"; WF_CFG=1.0; WF_STEPS=4;  WF_SCHEDULER=simple ;;
            flux-dev)     CKPT_NAME="${COMFYUI_DEFAULT_MODEL_NAME:-flux1-dev-fp8.safetensors}";   WF_CFG=1.0;  WF_STEPS=20; WF_SCHEDULER=simple ;;
            *) echo "Error: IMAGE_BACKEND=comfyui expects IMAGE_MODEL=sdxl|flux-schnell|flux-dev (got: $IMAGE_MODEL)"; exit 1 ;;
        esac
        export IMAGE_MODEL CKPT_NAME WF_CFG WF_STEPS WF_SCHEDULER
        ;;
    ollama)
        IMAGE_MODEL="${IMAGE_MODEL:-x/flux2-klein:4b}"
        # Ollama tags contain : (model:tag) or / (namespace/model). Reject
        # leftover ComfyUI presets like "sdxl" so the user notices the mismatch.
        if ! echo "$IMAGE_MODEL" | grep -qE '[:/]'; then
            echo "Error: IMAGE_BACKEND=ollama expects an Ollama model tag (e.g. x/flux2-klein:4b)."
            echo "       Got: $IMAGE_MODEL"
            exit 1
        fi
        export IMAGE_MODEL
        ;;
...

{
    ...
    if [ "$ENABLE_IMAGE_GENERATION" = "true" ]; then
        ...
        if [ "$IMAGE_BACKEND" = "ollama" ]; then
            echo "      - IMAGE_GENERATION_ENGINE=openai"
            echo "      - IMAGES_OPENAI_API_BASE_URL=http://ollama-image-bridge:8000/v1"
            echo "      - IMAGES_OPENAI_API_KEY=local"
            echo "      - IMAGE_GENERATION_MODEL=${IMAGE_MODEL}"
...
# ── Step 6: Image backend setup ──────────────────────────────
# comfyui: bootstrap + start ComfyUI natively, then warmup the checkpoint.
# ollama:  unload any leftover ComfyUI launchd agent so it doesn't sit in RAM,
#          and verify the bridge can reach Ollama (the bridge container itself
#          comes up in Step 7 via docker compose).
if [ "$ENABLE_IMAGE_GENERATION" = "true" ] && [ "$IMAGE_BACKEND" = "comfyui" ] && [ -x "./start-comfyui.sh" ]; then
    echo ""
    ./start-comfyui.sh || echo "Warning: ComfyUI failed to start — Open WebUI will run without image generation."
elif [ "$ENABLE_IMAGE_GENERATION" = "true" ] && [ "$IMAGE_BACKEND" = "ollama" ]; then
    if [ -x "./stop-comfyui.sh" ]; then
        ./stop-comfyui.sh > /dev/null 2>&1 || true
    fi
fi

# ── Step 6b: Pre-load the active checkpoint (comfyui only) ───
...
# cfg=1, steps=1 is just a load trigger and works for SDXL and Flux alike.
# (Ollama handles model loading on first /api/generate; no warmup needed.)
if [ "$ENABLE_IMAGE_GENERATION" = "true" ] && [ "$IMAGE_BACKEND" = "comfyui" ] && \
...
{"prompt":{"4":{"class_type":"CheckpointLoaderSimple","inputs":{"ckpt_name":"${CKPT_NAME}"}},"5":{"class_type":"EmptyLatentImage","inputs":{"width":128,"height":128,"batch_size":1}},"6":{"class_type":"CLIPTextEncode","inputs":{"text":"warmup","clip":["4",1]}},"7":{"class_type":"CLIPTextEncode","inputs":{"text":"","clip":["4",1]}},"3":{"class_type":"KSampler","inputs":{"seed":1,"steps":1,"cfg":1,"sampler_name":"euler","scheduler":"${WF_SCHEDULER}","denoise":1,"model":["4",0],"positive":["6",0],"negative":["7",0],"latent_image":["5",0]}},"8":{"class_type":"VAEDecode","inputs":{"samples":["3",0],"vae":["4",2]}},"9":{"class_type":"SaveImage","inputs":{"filename_prefix":"warmup","images":["8",0]}}}}
EOF
    then
        echo "ComfyUI warmup queued — ${IMAGE_MODEL} model loading in background."
    else
...
if [ "$OLLAMA_MODE" = "docker" ]; then
    COMPOSE_PROFILE_ARGS+=(--profile docker)
fi
if [ "$ENABLE_IMAGE_GENERATION" = "true" ] && [ "$IMAGE_BACKEND" = "ollama" ]; then
    COMPOSE_PROFILE_ARGS+=(--profile image-ollama)
fi
...
# ── Step 8: Patch image-gen config in WebUI DB ───────────────
# Open WebUI persists image-gen settings as PersistentConfig: env vars only
# seed first-boot, after that the sqlite row wins. So we patch the row
# directly. Sequence: (1) wait for DB init, (2) stop WebUI to flush its cache,
# (3) patch, (4) start WebUI.
if [ "$ENABLE_IMAGE_GENERATION" = "true" ]; then
...
if [ "$ENABLE_IMAGE_GENERATION" = "true" ] && [ "$IMAGE_BACKEND" = "ollama" ]; then
    # Point WebUI's "openai" image engine at the bridge. _managed_image_backend
    # marker triggers a repatch when the user switches backends.
    NEEDS_PATCH=$(IMAGE_MODEL="$IMAGE_MODEL" docker exec -i -e IMAGE_MODEL open-webui python - <<'PY' 2>/dev/null || echo "yes"
...
img = json.loads(row[0]).get('image_generation', {}) if row else {}
ok = (
    img.get('engine') == 'openai'
    and img.get('model') == os.environ['IMAGE_MODEL']
    and img.get('openai', {}).get('api_base_url') == 'http://ollama-image-bridge:8000/v1'
    and img.get('openai', {}).get('api_key') == 'local'
    and img.get('size') == '1024x1024'
    and img.get('_managed_image_backend') == 'ollama'
)
print("no" if ok else "yes")
PY
)
    if [ "$NEEDS_PATCH" = "yes" ]; then
        echo "Patching WebUI image config for IMAGE_BACKEND=ollama (model=$IMAGE_MODEL)..."
        ...
        docker run --rm -i \
            ...
            -v "${WEBUI_VOLUME}:/data" \
            python:3.12-alpine python - <<'PY' || echo "Warning: WebUI image patch failed — configure manually in Settings → Images."
import os, sqlite3, json
...
img['enable'] = True
img['engine'] = 'openai'
img['model'] = os.environ['IMAGE_MODEL']
img['size'] = '1024x1024'
img.setdefault('openai', {})
img['openai']['api_base_url'] = 'http://ollama-image-bridge:8000/v1'
img['openai']['api_key'] = 'local'
img['_managed_image_backend'] = 'ollama'
c.execute('UPDATE config SET data=? WHERE id=?', (json.dumps(data), cid))
db.commit()
PY
        docker compose ${COMPOSE_PROFILE_ARGS[@]+"${COMPOSE_PROFILE_ARGS[@]}"} start open-webui >/dev/null 2>&1
        for i in $(seq 1 30); do
...
if [ "$ENABLE_IMAGE_GENERATION" = "true" ] && [ "$IMAGE_BACKEND" = "comfyui" ]; then
    # SDXL requires 1024x1024 — at WebUI's default 512x512, SDXL produces
    # solid black images. Flux is distilled and needs cfg=1 + scheduler=simple,
    # which are baked into the workflow JSON, not exposed via WebUI controls.
    # We force size + workflow + node_ids together; the _managed_image_model
    # marker triggers a repatch when IMAGE_MODEL changes (e.g. sdxl → flux).
    NEEDS_PATCH=$(IMAGE_MODEL="$IMAGE_MODEL" docker exec -i -e IMAGE_MODEL open-webui python - <<'PY' 2>/dev/null || echo "yes"
import os, sqlite3, json
row = sqlite3.connect('/app/backend/data/webui.db').cursor().execute(
    "SELECT data FROM config ORDER BY id DESC LIMIT 1").fetchone()
...
model_ok = img.get('_managed_image_model') == os.environ.get('IMAGE_MODEL')
print("no" if (nodes_ok and size_ok and model_ok) else "yes")
PY
)
    if [ "$NEEDS_PATCH" = "yes" ]; then
        echo "Patching ComfyUI workflow for IMAGE_MODEL=$IMAGE_MODEL in WebUI..."
        ...
        docker run --rm -i \
            -e IMAGE_MODEL="$IMAGE_MODEL" \
            -e CKPT_NAME="$CKPT_NAME" \
            -e WF_CFG="$WF_CFG" \
            -e WF_STEPS="$WF_STEPS" \
            -e WF_SCHEDULER="$WF_SCHEDULER" \
            -v "${WEBUI_VOLUME}:/data" \
            python:3.12-alpine python - <<'PY' || echo "Warning: ComfyUI workflow patch failed — configure manually in WebUI Settings → Images."
import os, sqlite3, json
db = sqlite3.connect('/data/webui.db')
c = db.cursor()
row = c.execute('SELECT id, data FROM config ORDER BY id DESC LIMIT 1').fetchone()
...
cfg = float(os.environ['WF_CFG'])
steps = int(os.environ['WF_STEPS'])
scheduler = os.environ['WF_SCHEDULER']
...
workflow = {
    ...
    '9': {'class_type': 'SaveImage', 'inputs': {'filename_prefix': 'WebUI', 'images': ['8', 0]}},
}
...
img['steps'] = steps
img['model'] = ckpt
img.setdefault('comfyui', {})
img['comfyui']['workflow'] = json.dumps(workflow, indent=2)
img['comfyui']['nodes'] = [
...
img['_managed_image_model'] = os.environ['IMAGE_MODEL']
c.execute('UPDATE config SET data=? WHERE id=?', (json.dumps(data), cid))
db.commit()
PY
        docker compose ${COMPOSE_PROFILE_ARGS[@]+"${COMPOSE_PROFILE_ARGS[@]}"} start open-webui >/dev/null 2>&1
...
if [ "$ENABLE_IMAGE_GENERATION" = "true" ]; then
    if [ "$IMAGE_BACKEND" = "comfyui" ]; then
        echo "ComfyUI:    running natively on your host (localhost:${COMFYUI_PORT})"
    else
        echo "Image gen:  ollama-image-bridge → Ollama model ${IMAGE_MODEL}"
    fi
fi
